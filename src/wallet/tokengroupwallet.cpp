    ...
    CWallet *wallet)
{
...

    {
        ...
        // Add another input for the ION used for the fee
        ...
        // find a fee input
        std::vector<COutput> ioncoins;
        wallet->FilterCoins(ioncoins, [](const CWalletTx *tx, const CTxOut *out) {
            CTokenGroupInfo tg(out->scriptPubKey);
            return NoGroup == tg.associatedGroup;
        });

        COutput feeCoin(nullptr, 0, 0, false);
        if (!NearestGreaterCoin(ioncoins, fee, feeCoin))
        {
