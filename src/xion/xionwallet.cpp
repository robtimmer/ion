...
CxIONWallet::CxIONWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing xion seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadXIONSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseXIONSeed_deprecated()) {
                    LogPrintf("%s: Updated xION seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old xion seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of xion wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
void CxIONWallet::Lock()
{
    seedMaster = 0;
}

void CxIONWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
...
void CxIONWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToXION(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CxIONWallet::LoadMintPoolFromDB()
{
...
void CxIONWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
...
void CxIONWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->xionTracker->HasPubcoinHash(pMint.first)) {
...
bool CxIONWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToXION(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CxIONWallet::SeedToXION(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
void CxIONWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
