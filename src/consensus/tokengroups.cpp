...
CTokenGroupID BitcoinGroup(0);  // the "native" ion group id is one byte 0.  This is only used internally
...
CTokenGroupPair GetTokenGroupPair(const CScript& script)
{
    ...
    {
        LogPrint("tokens", "%s - empty script\n", __func__); // this should be impossible since that means script with 1 byte
        ret.associatedGroup = BitcoinGroup;
        return ret;
    }
