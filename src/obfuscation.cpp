...
bool CObfuscationPool::SignatureValid(const CScript& newSig, const CTxIn& newVin)
{
    ...
    if (found >= 0) { //might have to do this one input at a time?
        ...
        if (!VerifyScript(txNew.vin[n].scriptSig, sigPubKey, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, MAX_OPS_PER_SCRIPT, MutableTransactionSignatureChecker(&txNew, n))) {
