...
bool CXIonControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == XIonControlDialog::COLUMN_DENOMINATION || column == XIonControlDialog::COLUMN_VERSION || column == XIonControlDialog::COLUMN_CONFIRMATIONS)
...
void XIonControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CXIonControlWidgetItem* itemDenom(new CXIonControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CXIonControlWidgetItem *itemMint = new CXIonControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->xionTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->xionTracker->GetSpendCache(mint.hashStake);
